...
class UpcomingMoviesModel : BaseModel, MvpUpcomingMovies.Model {
    ...
    override fun getMovies(page: Int): Observable<MovieView> {
        return Observable.create<MovieView> { emitter : ObservableEmitter<MovieView> ->
            moviesRepository.getMovies("upcoming", page)
                ...
                .subscribeBy(
                    onNext = { movieModel : MovieModel ->
                        val date = try {
                            ...
                            formatter.parse(movieModel.releaseDate).toString()
                        } catch (e : Exception) {
                            null
                        }
                        val movie = MovieView(movieModel.id, movieModel.title, movieModel.overview,
                            movieModel.pathUrl, date, Category.Upcoming)
